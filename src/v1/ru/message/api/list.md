...
<br>Поиск может быть осуществлен по id и данным из полей кампании.
...
Для поиска можно использовать как одно поле, так и несколько полей одновременно.
 ...
 Параметр                     | Тип     | Описание
------------------------------|---------|-----------
`criteria[campaignIds]`         | array&nbsp;\|&nbsp;string | Поиск по идентификаторам кампаний <br>Параметр должен быть передан в виде массива или строки идентификаторов, разделенных запятыми. <br>Максимальное количество идентификаторов - 100, при превышении этого лимита поиск будет происходить по первым 100 id из списка
`criteria[from]`        | string | Поиск по подписи (альфаимени) отправителя <br>Поиск проходит по подписи отправителя, которая была создана в кампании.
`criteria[to]`       | string   | Поиск по номеру телефона получателя <br>Разрешен поиск как по целому номеру, так и по его части. <br>Например:  {widget:example-phone} <br>В поиске может участвовать только один номер.
...
`criteria[status]`                | integer | Поиск по [статусу сообщения](other#SmsStatus)
...
`criteria[campaignStatus]`            | string | Поиск по [статусу кампании](../campaign/list#list-campaign-statuses) SMS-сообщения  
...
<br>Например: 
<br>Сортировка по номеру получателя по возрастанию - `sort[to]`: ASC
<br>Сортировка по статусу сообщения по убыванию - `sort[status]`: DESC
