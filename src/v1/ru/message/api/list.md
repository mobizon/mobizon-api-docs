...
Поиск может быть осуществлен по [ID](other#glossary-id) и данным из полей кампании.
...
Для поиска можно использовать как одно поле, так и несколько полей одновременно.
 ...
 Параметр                     | Тип     | Описание
------------------------------|---------|-----------
`criteria[campaignIds]`         | array \ string | Поиск по идентификаторам кампаний.<br>Параметр должен быть передан в виде массива или строки идентификаторов, разделенных запятыми.<br>Максимальное количество идентификаторов – 100, при превышении этого лимита поиск будет происходить по первым 100 [ID](other#glossary-id) из списка.
`criteria[from]`        | string | Поиск по [подписи отправителя](other#glossary-sender-id).<br>Поиск проходит по подписи отправителя, которая была создана в кампании.
...
`criteria[groups]`              | string  | Поиск по идентификаторам [контактных групп](contact-groups), используемым в кампании.<br>Параметр должен быть передан в виде массива или строки идентификаторов, разделенных запятыми.
`criteria[campaignStatus]`            | string | Поиск по [статусу кампании](campaign#list-campaign-statuses) SMS-сообщения.  
...
`groups`            | string  | Идентификаторы [контактных групп](contact-groups), в которые входил номер получателя на момент создания кампании.
