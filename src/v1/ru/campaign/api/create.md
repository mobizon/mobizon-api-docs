...
`data[text]`                    | string  | Текст SMS сообщения для отправки <br>Для *шаблонной рассылки* (data[type]=3) этот текст должен содержать переменные, которые будут заменены на значения, уникальные для каждого получателя. Переменная должна быть написана символами латиницы, цифрами и символами `"_"` ,`"-"`, и обрамлена в фигурные скобки, например: {name} или {clientBalance}. <br>[Требования к тексту SMS](https://mobizon.ua/help/send-sms/sms-length-and-symbols). <br>В тексте сообщения вы можете использовать *короткие ссылки* и *функцию отслеживания получателей*, чтобы узнать кто из получателей перешел по вашей ссылке. 
...
`data[from]`                    | string  | *Подпись отправителя* <br>Для использования собственной подписи отправителя ее необходимо предварительно [зарегистрировать](https://mobizon.ua/help/sender-id/sender-id).<br>Если подпись не указана, будет использована подпись, установленная по умолчанию или стандартная подпись сервиса.
...
`data[trackShortLinkRecipients]`| integer |  *Функция отслеживания получателей* <br> Доступна к использованию только если в тексте сообщения (в поле `data[text]`) присутствует *короткая ссылка*, созданная в [нашем сервисе](https://mobizon.ua/help/url-shortener/how-to-shorten-url). <br>0 – не использовать функцию (установлено по умолчанию) <br>1 – использовать функцию
