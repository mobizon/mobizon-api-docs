...
`data[from]`                    | string  | [Подпись отправителя](other#glossary-sender-id).<br>Для использования собственной подписи отправителя ее необходимо предварительно [зарегистрировать](sender-id).<br>Если подпись не указана, будет использована подпись, установленная по умолчанию или стандартная подпись сервиса.
...
`data[trackShortLinkRecipients]`| integer |  [Функция отслеживания получателей](other#glossary-recipienttracking).<br>Доступна к использованию только если в тексте сообщения (в поле `data[text]`) есть [короткие ссылки](other#glossary-shortlink), созданные в [нашем сервисе](how-to-shorten-url).<br>**0** – не использовать функцию (установлено по умолчанию);<br>**1** – использовать функцию.
