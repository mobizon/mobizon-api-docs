...
<br>Статистика формируется с помощью различных счетчиков, которые отображаются в поле counters.
<br>**ВАЖНО!** 
<br>Из-за технических особенностей подсчета стоимости кампании, данные о возможной стоимости кампании указаны на момент добавления получателей и не меняются с течением времени. 
<br>Изменения могут произойти только при повторной загрузке получателей или при редактировании данных кампании (в случае редактирования данных все получатели и расчеты сбрасываются - требуется повторная загрузка).
...
`getFilledTplCampaignText` | bool    | Формат возвращаемых данных <br>**0** - текст кампании с плейсхолдерами <br>**1** - возвращать текст шаблонной кампании, заполненный реальными данными получателя (по умолчанию).
...
`isDeleted`                | integer | Флаг, который означает, что кампания удалена: <br>**0** - кампания доступна <br>**1**- кампания удалена
 ...
 `validity`                | integer | Максимальное время ожидания доставки сообщения, если получатель не может принять его сразу <br>Указывается в минутах: от 60 минут (1 час) до 1440 минут (24 часа) с момента отправки.
 ...
 `creationWay`                | integer  | Способ создания кампании: <br>**1** - с помощью Интернет-браузера <br>**5** - функциональная (служебная) кампания (ссылка на термин в глоссарии)
 `counters`                | object  | Различные счетчики кампании, описанные в таблице **Поля объекта counters**
...
`totalSegNum`        | integer  | Общее количество сегментов в кампании. <br>Обновляется при добавлении получателей.
...
`recipientsRejected` | integer  | Количество отклоненных получателей (не включенных в кампанию) <br>Обновляется при добавлении получателей.
